def main():
        ...
        while(True):
                ...
                if seasonal:
                        ...
                        day_period, period_duration, period_start_time = day_period_params(now, sunrise_start, sunrise_end, sunset_start, sunset_end, next_sunrise_start)                       

                        if day_period != last_day_period:
                                file_list = load_file_list(pathname, image_format, day_period)
    
                        selected_image, time_per_image = select_image_seasonal(pathname, day_period, file_list, period_duration, period_start_time, now)
...
def select_image_seasonal(pathname, day_period, file_list, period_duration, period_start_time, now):
        ...
        index_number = min(int(round((now - period_start_time).total_seconds() / time_per_image,0)), len(file_list) - 1)
...
def select_image_nonseasonal(file_list, now_seconds):
        ...
        filepath = min(file_list, key = lambda x: abs(x-int(math.ceil(now_seconds,0))))
...
def set_wallpaper(filepath, wallpaper_option):
        ...
        import iad
        wallpaper_options_dict = {"center": 0x0, "tile": 0x1, "stretch": 0x2, "fit": 0x3, "fill": 0x4, "span": 0x5} # https://docs.microsoft.com/en-us/windows/win32/api/shlobj_core/ns-shlobj_core-wallpaperopt
