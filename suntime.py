...
def suntime(day_offset: int, lat: str, lon: str, elevation: int = 0):
    ...
    home.date = datetime.combine(datetime.now().date(), time(00,00,00)) - timedelta(seconds = utc_offset) - timedelta(days = day_offset)
    
    print("Evaluating sunrise/sunset times for: %s UTC%+d" % ((home.date.datetime() + timedelta(seconds = utc_offset)).date(), utc_offset/3600))

    # Sunrise starting time
    sunrise_start = home.next_rising(sun).datetime() + timedelta(seconds = utc_offset)
    ...
    # Sunset ending time
    sunset_end = home.next_setting(sun).datetime() + timedelta(seconds = utc_offset)
